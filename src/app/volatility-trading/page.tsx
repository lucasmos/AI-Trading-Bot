...
export default function VolatilityTradingPage() {
  ...
  const handleAccountTypeSwitch = async (newTypeFromControl: 'demo' | 'real' | null) => {
    ...
    // Using the condition from feat/volatility-ai-trading-loop as it's more specific to account IDs
  ...
  const handleStartAiAutoTrade = useCallback(async () => {
    ...
    // Low stake warning from feat/volatility-ai-trading-loop
    ...
    // For new real trading loop (from feat/volatility-ai-trading-loop)
    if (selectedUserTradeTypeForLoop) {
    ...
    } else { // Fallback to OLD SIMULATION logic (from feat/volatility-ai-trading-loop, which is similar to main's original logic)
        ...
        try {
            ...
            for (const proposal of strategyResult.tradesToExecute) {
              ...
              const stopLossPercentage = 0.05; // Standard stop-loss for simulation
              // Ensure actionDirection is derived from proposal.action for simulation
              ...
              newTrades.push({
                ...
                derivContractType: proposal.action, // 'CALL' or 'PUT'
                userSelectedTradeType: "RiseFall", // Simulation implies Rise/Fall
                actionDirection: actionDirection, // Store this for simulation logic
    ...
    // setProfitsClaimable is not needed here as it's handled by localStorage effect and trade completion logic
]);
    ...
    setActiveAutomatedTrades(prevTrades =>
      prevTrades.map(trade => {
        if (trade.status === 'active') {
          const pnl = -trade.stake; // Assume loss if manually stopped during simulation
          if (userInfo?.id && !selectedUserTradeTypeForLoop) { // Only log for simulation stops
            console.log('[VolatilityPage] Storing manually stopped SIMULATED trade for user:', userInfo.id);
            fetch('/api/trades', { // Using main's DB logging structure
              method: 'POST',
              ...
              body: JSON.stringify({
                ...
                type: trade.actionDirection === 'CALL' ? 'buy' : 'sell', // Use actionDirection for sim
                amount: trade.stake,
          ...
          setProfitsClaimable(prevProfits => ({
            ...
            winningTrades: prevProfits.winningTrades, // No win on manual stop
            losingTrades: prevProfits.losingTrades + 1,
          }));
          // Using 'closed_manual' from feat/volatility-ai-trading-loop as it's more descriptive
        ...
        const intervalId = setInterval(() => {
          setActiveAutomatedTrades(prevTrades => {
            ...
            const updatedTrades = prevTrades.map(currentTrade => {
              ...
              // Using actionDirection for simulation logic as per merged handleStartAiAutoTrade
              ...
              if (newStatus === 'active' && Date.now() >= currentTrade.startTime + currentTrade.durationSeconds * 1000) {
                const isWin = Math.random() < 0.83; // Simulation win rate
              ...
              if (newStatus !== 'active') {
                ...
                if (userInfo?.id) { // DB Logging from main
                  ...
                  fetch('/api/trades', {
                    ...
                    body: JSON.stringify({
                      ...
                      type: currentTrade.actionDirection === 'CALL' ? 'buy' : 'sell', // Use actionDirection
        ...
        <div className="md:col-span-1">
          <Card className="shadow-lg">
            ...
            <CardContent className="space-y-6">
              ...
              {/* New Selector for UserTradeType for REAL TRADING LOOP (from feat/volatility-ai-trading-loop) */}
              <div className="space-y-2">
                <Label htmlFor="volatility-user-trade-type-loop">Select Trade Type (for Real Backend Loop)</Label>
                <Select
                  value={selectedUserTradeTypeForLoop || ""} // Ensure value is not undefined for Select
                  onValueChange={(value) => setSelectedUserTradeTypeForLoop(value as UserTradeTypeValue | undefined)}
                  ...
                  <TableBody>
                    {activeAutomatedTrades.map(trade => (
                      <TableRow key={trade.id}>
                        ...
                        {selectedUserTradeTypeForLoop ? ( // Display for Real Backend Loop Trades
                          <>
                            <TableCell>
                        ...
                        ) : ( // Display for Page Simulation Trades
                          <>
...
// Assign uuidv4 to window if in browser environment (for potential direct script uses, though typically handled by module system)
